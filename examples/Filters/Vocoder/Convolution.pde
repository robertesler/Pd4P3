class Convolution extends PdMaster {
 
   rFFT rfft = new rFFT();//for our input, aka filter
   rIFFT rifft = new rIFFT();
   rFFT rfft2 = new rFFT();//for our Synth, aka control
   ControlFFT controlFFT = new ControlFFT();
  ...
  float doFFT(float filter, float control) {
    ...
    if(sampleCounter == hop)
    {
      ...
      for(int i = 0; i < this.getFFTWindow(); i++)
      {
        ...
        //thread our 2nd FFT
        if(ifThreading)
          controlFFT.setInput(buffer2.get(i) * hann[i]);
      }
...
public float rsqrt(float x) {
  ...
  //We use synchronized to communicate with the audio thread
   ...
   void free() {
     ...
     Oscillator.free(osc);
     if(ifThreading)
       controlFFT.stop();//this also frees our memory in the class
   }
