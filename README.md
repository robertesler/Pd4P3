# Pd4P3
A real-time audio synthesis library for Processing [https://processing.org/]. Pd4P3 implements Pure Data's signal processing objects via Pd++ native code.  Pd4P3 stands for "Pd++ for Processing 3".
...
# P3 library
You can download the Processing  library via the Contribution Manager.
...
# Versions (Win/MacOS/Linux/Rpi/Android/Unity/Unreal/FMOD/Wwise)
This version of the library right now works on Windows 10/11, MacOS, Linux Ubuntu 20.x., Android and Unity.  It has also been tested successfully in plugins for Unreal Engine's Metasound, FMOD (Plugin Instrument) and Wwise (Source Plugin).  The .dll in the /library folder are built for Windows, the .dylibs are for MacOS, the .so for linux and Android.  The linux version does not have support for PortAudio right now.  This library has also been tested on the Raspberry Pi and works as expected based on the limitations of the hardware.  I am able to get full duplex audio on an Rpi 3 with an occasional audio interruption. More details below on how to build for the Rpi.

We are also currently testing C# bindings in the Unity game engine. So far it tests successfully in Unity 6.x.x for MacOS and Windows.  You can see more here: https://github.com/robertesler/Pd4Unity.  Since Unreal Engine, FMOD and Wwise use C++ the, core library (Pd++) works as expected in each respective engine or middleware plugin API.  You can see examples here: https://github.com/robertesler/PdUnreal/.  However, Unity's C# scripting environment is much closer to Processing's, it uses methods similar to ```setup(), draw()``` via the ```Start(), Update()```, respectively.  
